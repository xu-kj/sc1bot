...
void MapTools::computeMap()
{
	...
	for (size_t x = 0; x < m_map.width(); x++)
	{
		...
		{
			if (isDepotBuildableTile(x, y))
			{
				m_map.set(x, y, TileType::BuildAll);
			}
			else if (isBuildable(x, y))
			{
				m_map.set(x, y, TileType::NoDepot);
			}
			else if (isWalkable(x, y))
			{
				m_map.set(x, y, TileType::Walk);
			}
			else
			{
				m_map.set(x, y, TileType::Unwalkable);
			}
		}
	}
	...
	for (int x = sx; x < ex; ++x)
	{
		for (int y = sy; y < ey; y++)
		{
			...
			if (Config::Debug::DrawTileInfo)
			{
				...
				{
					color = BWAPI::Color(255, 255, 0);
				}
				if (isBuildable(x, y) && !isDepotBuildableTile(x, y))
				{
					color = BWAPI::Color(127, 255, 255);
				}
				drawTile(x, y, color);
			}
		}
